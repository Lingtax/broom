#' Tidying methods for linear models with interactions as generated by the package pequod.
#' Code is derived from lm_tidiers.R 
#'
...
#'
#' @seealso \code{\link{summary.lmres}}
#'
#' @name lmres_tidiers
#' 
#' @param x lmres object
...
#' library(dplyr)
#'
#' mod <- lmres(mpg ~ wt*qsec, data = mtcars, centred =c("wt", "qsec"))
#' 
#' tidy(mod)
#' glance(mod)
#' 
...
#' \code{glance} are not.
#' 
#' @return \code{tidy.lmres} returns one row for each coefficient, with five columns:
...
#' 
#' @export
tidy.lmres <- function(x, conf.int = FALSE, conf.level = .95,
                    exponentiate = FALSE, quick = FALSE, ...) {
    if (quick) {
        co <- stats::coef(x)
        ret <- data.frame(term = names(co), estimate = unname(co))
        return(process_lmres(ret, x, conf.int = FALSE, exponentiate = exponentiate))
    }
    s <- summary(x)
    ret <- tidy.summary.lmres(s)
    
    process_lmres(ret, x, conf.int = conf.int, conf.level = conf.level,
...
#' 
#' @details Code and documentation for \code{augment.lmres} originated in the
#' ggplot2 package, where it was called \code{fortify.lmres}
#' 
#' @return When \code{newdata} is not supplied \code{augment.lmres} returns
...
#' 
#' 
#' When \code{newdata} is supplied, \code{augment.lmres} returns one row for each
...
#' 
#' @export
augment.lmres <- function(x, data = stats::model.frame(x), newdata,
...
#' 
#' @return \code{glance.lmres} returns a one-row data.frame with the columns
...
#' 
#' @export
glance.lmres <- function(x, ...) {
    # use summary.lmres explicity, so that c("aov", "lm") objects can be
    # summarized and glanced at
    s <- stats::summary.lmres(x)
    ret <- glance.summary.lmres(s, ...)
...
#' @rdname lmres_tidiers
#' @export
glance.summary.lmres <- function(x, ...) {
...
#' helper function to process a tidied lmres object
#' 
...
#' @param exponentiate whether to exponentiate the coefficient estimates
#' and confidence intervals (typical for logistic regression)
process_lmres <- function(ret, x, conf.int = FALSE, conf.level = .95,
                       exponentiate = FALSE) {
    if (exponentiate) {
